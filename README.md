...
### Reflections
This one took twice as long as it should have due to some confusion over the dynamics of the puzzle pieces.  I intended — as one mindful of the physical rigidity of a real-world shape — that the assignments of pips to pieces was tied to the cells in the shape, but Codex (somehow?) got the idea that it was allowed to permute the pips within the piece, which ballooned the search space.  This also caused a subtle bug in the first (apparently working) version of the generation code that produced some unsolvable puzzles with the pieces as notated.  (The pieces were improperly normalized when output.)

With the solver taking an unreasonably long time (tens of minutes) to not complete a puzzle where the combination of the geometry of the pieces and the arithmetic constraints should have resulted in a small search space, I had Codex go back to the drawing board ("v2 solver", if references remain in the code) with more emphasis on ensuring that the piece notation and other fundamental aspects of the project met my expectations.
